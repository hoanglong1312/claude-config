...
Hỏi 1 lần: "Bật `bypassPermissions` cho project không? Claude sẽ không hỏi confirm mỗi tool call."

Nếu có → ghi vào `.claude/settings.json`:
```json
{
  "permissions": {
...
Nếu `.claude/settings.json` đã tồn tại → merge key `permissions.bypassPermissions`, không replace file. Validate: `jq empty .claude/settings.json`.
...
Hỏi 1 lần: "Stamp slash commands vào `.claude/commands/` không?"

**Luôn stamp (code project):**
- Copy từ `~/.claude/templates/commands/codex-exec.md` → `.claude/commands/codex-exec.md`
- Copy từ `~/.claude/templates/commands/plan-track.md` → `.claude/commands/plan-track.md`
- Tạo `review.md`, `verify.md`, `ship.md` từ Component Templates (bên dưới)

Nếu project đã có test runner riêng (pytest / vitest / go test...) → điền đúng lệnh vào `verify.md`.

**Stamp theo stack detect:**
...
Auto-detect từ deps (hỏi gộp 1 lần):
...
Manual opt-in (không auto-detect, chỉ thêm khi user yêu cầu):
- `ui-design-handoff.md` — task design UI/landing/redesign
- `codex-html-workflow.md` — plan/spec render HTML
...
**3i. Statusline (global, mọi loại project):**

```bash
cat ~/.claude/settings.json | python3 -c "import json,sys; d=json.load(sys.stdin); print('SET' if d.get('statusLine') else 'NOT SET')" 2>/dev/null
```

Nếu chưa set → hỏi: "Muốn cấu hình statusline (model + rate limits bar) không?" → invoke `statusline-setup` agent.
